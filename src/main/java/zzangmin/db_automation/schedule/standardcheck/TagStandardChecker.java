            ...
            for (DBCluster cluster : accountDbClusters) {
                ...
                for (String tagName : standardTagKeyNames) {
                    if (!clusterTagKeys.contains(tagName)) {
                        results.add(new StandardCheckResultResponseDTO(accountId, cluster.dbClusterIdentifier(), StandardCheckResultResponseDTO.StandardType.TAG, tagName, "태그 생성 필요", null, "표준 태그가 존재하지 않습니다."));
                    }
                }
            }
        }
            ...
            for (DBInstance dbInstance : accountDbInstances) {
                ...
                for (String tagName : standardTagKeyNames) {
                    if (!instanceTagKeys.contains(tagName)) {
                        results.add(new StandardCheckResultResponseDTO(accountId, dbInstance.dbInstanceIdentifier(), StandardCheckResultResponseDTO.StandardType.TAG, tagName, "태그 생성 필요", null, "표준 태그가 존재하지 않습니다."));
                    }
                }
            }
        }
